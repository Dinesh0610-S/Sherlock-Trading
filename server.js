...
app.get('/api/nse/quote', async (req, res) => {
  ...
  try {
    ...
    const mockQuotes = {
      'NIFTY': { lastPrice: 23242.10, dayHigh: 23350.0, dayLow: 23150.0, previousClose: 23200.0, change: 42.10, changePct: 0.18 },
      'BANKNIFTY': { lastPrice: 55194.50, dayHigh: 55400.0, dayLow: 55000.0, previousClose: 55100.0, change: 94.50, changePct: 0.17 },
      'RELIANCE': { lastPrice: 1269.20, dayHigh: 1280.0, dayLow: 1260.0, previousClose: 1265.0, change: 4.20, changePct: 0.33 }
    };
...
// Spec: https://github.com/tradingview/charting_library/wiki/UDF
// These routes feed the lightweight-charts custom datafeed in TradingViewChart.jsx
// ─────────────────────────────────────────────────────────────────────────────

// UDF resolution → Yahoo Finance interval + range mapping
const UDF_RESOLUTION_MAP = {
  '1':   { interval: '1m',  range: '1d'  },
  '5':   { interval: '5m',  range: '1d'  },
  '10':  { interval: '5m',  range: '5d'  },  // aggregated 5m→10m server-side
...
// Internal mapping: UDF resolution → our /api/candles interval param
const UDF_TO_CANDLES_INTERVAL = {
  '1': '1m', '5': '5m', '10': '10m', '15': '15m',
  '30': '30m', '60': '1h', 'D': '1d', '1D': '1d', 'W': '1d',
};

// Supported symbols for UDF search
const UDF_SYMBOLS = [
  { symbol: 'NIFTY',      full_name: 'NSE:NIFTY_50',    description: 'NIFTY 50 Index',         exchange: 'NSE', type: 'index'  },
  { symbol: 'BANKNIFTY',  full_name: 'NSE:BANKNIFTY',   description: 'Bank NIFTY Index',        exchange: 'NSE', type: 'index'  },
  { symbol: 'FINNIFTY',   full_name: 'NSE:FINNIFTY',    description: 'NIFTY Financial Services', exchange: 'NSE', type: 'index'  },
  { symbol: 'SENSEX',     full_name: 'BSE:SENSEX',      description: 'BSE SENSEX Index',        exchange: 'BSE', type: 'index'  },
  { symbol: 'MIDCPNIFTY', full_name: 'NSE:MIDCPNIFTY',  description: 'NIFTY Midcap 50',         exchange: 'NSE', type: 'index'  },
  { symbol: 'RELIANCE',   full_name: 'NSE:RELIANCE',    description: 'Reliance Industries Ltd', exchange: 'NSE', type: 'stock'  },
  { symbol: 'HDFCBANK',   full_name: 'NSE:HDFCBANK',    description: 'HDFC Bank Ltd',           exchange: 'NSE', type: 'stock'  },
  { symbol: 'ICICIBANK',  full_name: 'NSE:ICICIBANK',   description: 'ICICI Bank Ltd',          exchange: 'NSE', type: 'stock'  },
  { symbol: 'INFY',       full_name: 'NSE:INFY',        description: 'Infosys Ltd',             exchange: 'NSE', type: 'stock'  },
  { symbol: 'TCS',        full_name: 'NSE:TCS',         description: 'TCS Ltd',                 exchange: 'NSE', type: 'stock'  },
  { symbol: 'WIPRO',      full_name: 'NSE:WIPRO',       description: 'Wipro Ltd',               exchange: 'NSE', type: 'stock'  },
  { symbol: 'SBIN',       full_name: 'NSE:SBIN',        description: 'State Bank of India',     exchange: 'NSE', type: 'stock'  },
];

// ── GET /udf/config ────────────────────────────────────────────────────────
// Returns UDF server configuration: supported resolutions, exchanges, types.
// Datafeed's onReady() callback receives this to know what the server can handle.
...
// ── GET /udf/time ─────────────────────────────────────────────────────────
// Returns current server Unix timestamp. Used by the datafeed for time sync.
app.get('/udf/time', (req, res) => {
  res.send(String(Math.floor(Date.now() / 1000)));
});

// ── GET /udf/search ───────────────────────────────────────────────────────
// Symbol search endpoint. Used by the chart's symbol search UI.
app.get('/udf/search', (req, res) => {
  const { query = '', type = '', limit = 30 } = req.query;
  const q = query.toUpperCase();
  const results = UDF_SYMBOLS
    .filter(s =>
...
// ── GET /udf/symbols ──────────────────────────────────────────────────────
// Resolves a symbol to its full metadata for the chart.
// resolveSymbol() in the datafeed calls this.
app.get('/udf/symbols', (req, res) => {
  // symbol param can be "NIFTY", "NSE:NIFTY_50", etc.
  const rawSymbol = (req.query.symbol || 'NIFTY').toUpperCase()
    .replace('NSE:', '').replace('BSE:', '')
    .replace('_50', '').replace('NIFTY 50', 'NIFTY');

  const cleanSym = rawSymbol.trim();

  // Default pricescale: indices use 100 (2 decimal), stocks use 100
  const isIndex   = ['NIFTY', 'BANKNIFTY', 'FINNIFTY', 'SENSEX', 'MIDCPNIFTY'].includes(cleanSym);
  const basePrice = { NIFTY: 23000, BANKNIFTY: 51000, SENSEX: 75000 }[cleanSym] || 23000;

  res.json({
    name:              cleanSym,
    ticker:            cleanSym,
    description:       UDF_SYMBOLS.find(s => s.symbol === cleanSym)?.description || cleanSym,
    type:              isIndex ? 'index' : 'stock',
    session:           '0915-1530',            // NSE/BSE trading hours IST
    holidays:          '',                      // Add NSE holiday dates here if needed
    corrections:       '',
    timezone:          'Asia/Kolkata',
    exchange:          ['SENSEX'].includes(cleanSym) ? 'BSE' : 'NSE',
    listed_exchange:   ['SENSEX'].includes(cleanSym) ? 'BSE' : 'NSE',
    minmov:            5,                       // minimum price movement
    pricescale:        100,                     // display 2 decimal places
    minmov2:           0,
    fractional:        false,
    has_intraday:      true,
    has_no_volume:     false,
    ...
    // Metadata for the chart header display
    'exchange-traded': ['SENSEX'].includes(cleanSym) ? 'BSE' : 'NSE',
    'exchange-listed': ['SENSEX'].includes(cleanSym) ? 'BSE' : 'NSE',
    full_name:         `${['SENSEX'].includes(cleanSym) ? 'BSE' : 'NSE'}:${cleanSym}`,
  });
});

// ── GET /udf/history ──────────────────────────────────────────────────────
// Historical bar data endpoint — the heart of the UDF protocol.
// getBars() in the datafeed calls this and receives UDF table format.
//
// ╔══════════════════════════════════════════════════════════════════╗
// ║  LIVE BROKER API INTEGRATION POINT                              ║
// ║  To connect a real broker (Zerodha/Angel/TrueData):            ║
// ║  1. Map `symbol` + `resolution` to the broker's instrument key ║
// ║  2. Call the broker's historical data API                       ║
// ║  3. Parse their OHLCV arrays into the UDF table format below   ║
// ║                                                                 ║
// ║  Zerodha Kite example:                                         ║
// ║    kite.getHistoricalData(instrumentToken, interval, from, to) ║
// ║    → map response.data to { t, o, h, l, c, v } arrays         ║
// ╚══════════════════════════════════════════════════════════════════╝
app.get('/udf/history', async (req, res) => {
  const { symbol = 'NIFTY', resolution = '15', from, to } = req.query;

  // Map UDF resolution → our candle interval param
  const interval = UDF_TO_CANDLES_INTERVAL[resolution] || '15m';
  const cleanSym = symbol.toUpperCase().replace('NSE:', '').replace('BSE:', '')
    .replace('_50', '').replace('NIFTY 50', 'NIFTY').trim();

  const CACHE_KEY = `udf_${cleanSym}_${interval}`;
  const cached = cache.get(CACHE_KEY);
  ...
  try {
    ...
    if (cached) {
      ...
      // Re-use same Yahoo Finance fetch logic as /api/candles
      // ── BROKER API SWAP POINT: replace this block with your broker fetch ──
      const yahooInterval =
        interval === '1m'  ? '1m'  :
        interval === '5m'  ? '5m'  :
        interval === '10m' ? '5m'  :
      ...
      const yahooSymbol = getYahooSymbol(cleanSym);
      const yahooUrl = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(yahooSymbol)}?interval=${yahooInterval}&range=${range}&includePrePost=false`;
      ...
      // Aggregate 5m → 10m if resolution is "10"
      if (interval === '10m') {
        const groups = {};
        candles.forEach(c => {
        ...
        candles = Object.keys(groups).map(Number).sort((a, b) => a - b).map(key => {
          const g = groups[key];
          return {
            time:   key,
            open:   g[0].open,
      ...
      // Cache with appropriate TTL per resolution
      const ttl = ['1', '5'].includes(resolution) ? 10
                : resolution === '15' ? 60
                : resolution === '30' ? 120
                : resolution === '60' ? 300 : 3600;
      cache.set(CACHE_KEY, candles, ttl);
    }

    // Filter by requested from/to window
    const fromTs = from ? Number(from) : 0;
    const toTs   = to   ? Number(to)   : Infinity;
    const filtered = candles.filter(c => c.time >= fromTs && c.time <= toTs);

    if (filtered.length === 0) {
      // UDF "no data" response — chart will show empty bars
      return res.json({ s: 'no_data' });
    }

    // ── UDF Table Format ──────────────────────────────────────────────────
    // This is the exact format TradingView's getBars() callback expects.
    // If integrating a live broker API, parse their response into these arrays:
    //   t[i] = UNIX timestamp (seconds)
    //   o[i] = open price
    //   h[i] = high price
    //   l[i] = low price
    //   c[i] = close price
    ...
    console.warn(`[UDF /history] ${cleanSym} ${resolution}: ${err.message}`);
    // Return "no_data" so the chart handles gracefully instead of crashing
    res.json({ s: 'no_data' });
  }
});
...
// Confidence score calculator (10-Dimension Engine)
function calculatePreMarketConfidence(d) {
  ...
  const trendCtx = buildTrendContext(d.candles15m || []);

  const scan = scanNiftyPatterns(d.candles15m || [], {
    rsi15m:       d.rsi15m,
    macdRising:   d.macdRising,
    macdHist:     d.macdHist,
    volRatio:     d.volumeRatio || 1.0,
    aboveVwap:    d.vwapVal ? (d.spot > d.vwapVal) : false,
    trend15m:     trend15m.label,
    trend1h:      trend1h.label,
    atSupport:    trendCtx.atSupport,
    atResistance: trendCtx.atResistance,
    newsScore:    d.newsScore || 0,
  }, trend15m.label);
  d.scanResultObj = scan; // Store it on d for later access at the end of the function

  dims.push({
    name:  `Pattern Engine (${scan.allPatterns.length} patterns)`,
    score: Math.min(scan.patternScore * 1.5, 15),
    max:   15,
    note:  scan.summary,
  });
...
app.post('/api/premarket/options-entry', async (req, res) => {
  ...
  try {
    ...
    // Compute newsScore from headlines
    ...
    const confidenceObj = calculatePreMarketConfidence({
      ...
      // UPGRADED PATTERN ENGINE INPUTS
      candles15m: candles,
      newsScore: newsScore,
